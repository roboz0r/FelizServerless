...
module FuncEngDB =
    ...
    let inline private encoder<'T> extraCoders =
        Encode.Auto.generateEncoderCached<'T> (CamelCase, extraCoders, false)

    let inline private decoder<'T> extraCoders =
    ...
    let private serialiser =
        ...
        { new CosmosSerializer() with
            ...
            member __.ToStream<'T>(input: 'T) =
                ...
                use streamWriter =
                    new StreamWriter(streamPayload, encoding = utf8, bufferSize = 1024, leaveOpen = true)

                use writer =
                    new Newtonsoft.Json.JsonTextWriter(streamWriter)
